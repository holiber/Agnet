# Post videos of scenarios on gh pages
```yml
...
jobs:
  build_and_deploy:
    ...
    steps:
      ...
      # Fail fast if GitHub Pages is not enabled
      ...
      # Web scenarios: Playwright browsers + OS deps
      - name: Install Playwright (with deps)
        run: npx playwright install --with-deps

      # CLI scenario video tooling
      - name: Install asciinema + ffmpeg
        run: |
          sudo apt-get update
          sudo apt-get install -y asciinema ffmpeg

      # cast -> gif -> mp4 conversion
      ...
      - name: Build GH Pages content (/scenarios)
        ...
        run: |
          ...
          # Copy recorded artifacts into the site under /scenarios/videos/...
          if [ -d "artifacts/user-style-e2e" ]; then
            cp -R "artifacts/user-style-e2e" "$SCENARIOS_DIR/videos/"
          fi

          # Root index: link to /scenarios
          ...
          <body>
            ...
            <div class="meta">
              This page contains user-like scenario test videos generated from the latest merge to <code>main</code>.
            </div>
              ...
              async function main() {
                ...
                if (!res.ok) {
                  document.body.insertAdjacentHTML("beforeend",
                    "<p><b>No manifest.json found.</b> Videos may be missing.</p>");
                  return;
                }
          ...
          # Build /scenarios/manifest.json by scanning videos
          node - << 'NODE'
          const fs = require("fs");
          ...
          function rel(p) {
            return path.relative(scenariosDir, p).replaceAll(path.sep, "/");
          }
